...
class AgentEngine(str, Enum):
    """Agent engine to use for conversation."""
    OPENHANDS_SDK = 'openhands_sdk'   # Default: OpenHands SDK agent loop
    CLAUDE_SDK = 'claude_sdk'          # Claude Agent SDK (Claude CLI subprocess)
# >>> END CUSTOM <<<
...
class _StartConversationRequestBase(BaseModel):
    ...
    # >>> CUSTOM: HiClaw <<<
    agent_engine: AgentEngine = Field(
        default=AgentEngine.OPENHANDS_SDK,
