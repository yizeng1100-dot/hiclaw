...
async def _refresh_next_fire_at_column() -> None:
    ...
    Invoked by :func:`start` once at boot and by the router after any
    CRUD. The list page reads this column directly so it doesn't need
    a live RPC to the scheduler.
    """
    sched = _get_scheduler()
    db = await get_agent_db()
    try:
        ...
        for job in sched.get_jobs():
            ...
            await svc.set_next_fire_at(job.id, nrt)
    finally:
        await db.close()
