...
# >>> CUSTOM: HiClaw — remote OS type for multi-OS provisioning <<<
class OsType(str, Enum):
    """Operating system of the remote worker machine.

    Selects which Provisioner subclass to use (LinuxProvisioner vs
    WindowsProvisioner). Default is LINUX for backwards compatibility —
    all pre-existing connect requests will continue to hit the Linux code
    path untouched.
    """
...
class ConnectMachineRequest(BaseModel):
    ...
    # >>> CUSTOM: HiClaw — OS type selects Linux vs Windows provisioner <<<
...
class MachineInfo(BaseModel):
    ...
    template: str
    workspace: str
    # >>> CUSTOM: HiClaw — OS type is sticky once a machine is connected <<<
    os_type: OsType = OsType.LINUX
    # >>> END CUSTOM <<<
