...
class Provisioner:
    ...
    async def _write_wrapper_scripts(self, venv: str, remote_python: str) -> None:
        ...
        await self.ssh.run(
            ...
            "    except Exception:\n"
            "        pass\n"
            "# Force httpx connect timeout — prevents hanging when proxy accepts TCP but stalls\n"
            "try:\n"
            "    import httpx\n"
            "    httpx._config.DEFAULT_TIMEOUT_CONFIG = httpx.Timeout(connect=10, read=60, write=30, pool=10)\n"
            "    print('[HiClaw] httpx default timeout set: connect=10s read=60s', file=sys.stderr)\n"
            "except Exception as e:\n"
            "    print(f'[HiClaw] httpx timeout patch failed: {e}', file=sys.stderr)\n"
            "# Register litellm failure callback to log full error chain\n"
            "try:\n"
            "    import litellm\n"
            "    def _hiclaw_failure_hook(kwargs, exception, start_time, end_time):\n"
            "        elapsed = (end_time - start_time).total_seconds() if start_time and end_time else '?'\n"
            "        print(f'[LLM_FAILURE] model={kwargs.get(\"model\")}, base_url={kwargs.get(\"api_base\")}, elapsed={elapsed}s')\n"
            "        print(f'[LLM_FAILURE] exception_type={type(exception).__name__}')\n"
            "        print(f'[LLM_FAILURE] exception={exception}')\n"
            "        cause = exception.__cause__\n"
            "        depth = 0\n"
            "        while cause and depth < 5:\n"
            "            print(f'[LLM_FAILURE] cause[{depth}]={type(cause).__name__}: {cause}')\n"
            "            cause = cause.__cause__\n"
            "            depth += 1\n"
            "        if hasattr(exception, 'response') and exception.response is not None:\n"
            "            r = exception.response\n"
            "            print(f'[LLM_FAILURE] status={getattr(r, \"status_code\", \"?\")}')\n"
            "            print(f'[LLM_FAILURE] body={getattr(r, \"text\", \"\")[:2000]}')\n"
            "        print(f'[LLM_FAILURE] headers_sent={kwargs.get(\"headers\", {})}')\n"
            "    def _hiclaw_pre_hook(model, messages, kwargs):\n"
            "        import datetime\n"
            "        print(f'[LLM_CALL] {datetime.datetime.now().isoformat()} model={model}, base_url={kwargs.get(\"api_base\", \"?\")}, timeout={kwargs.get(\"timeout\", \"?\")}')\n"
            "        return kwargs\n"
            "    litellm.input_callback = [_hiclaw_pre_hook]\n"
            "    litellm.failure_callback = [_hiclaw_failure_hook]\n"
            "    print('[HiClaw] litellm callbacks registered', file=sys.stderr)\n"
            "except Exception as e:\n"
            "    print(f'[HiClaw] Failed to register litellm callback: {e}', file=sys.stderr)\n"
