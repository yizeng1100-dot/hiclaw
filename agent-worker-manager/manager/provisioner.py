...
class Provisioner:
    ...
    async def _write_wrapper_scripts(self, venv: str, remote_python: str) -> None:
        ...
        await self.ssh.run(
            ...
            "#\n"
            "# Force httpx connect timeout (proxy can accept TCP but stall HTTP)\n"
            "try:\n"
            "    import httpx\n"
            ...
            "    pass\n"
            "#\n"
            "# Monkey-patch SDK retry_mixin to print full error cause chain\n"
            "try:\n"
            "    from openhands.sdk.llm.utils import retry_mixin as _rm\n"
            "    _orig_log = _rm.RetryMixin.log_retry_attempt\n"
            "    def _patched_log(self, retry_state):\n"
            "        _orig_log(self, retry_state)\n"
            ...
            "            print(f'[LLM_ERROR] cause[{depth}] {type(cause).__name__}: {cause}', flush=True)\n"
            "            cause = getattr(cause, '__cause__', None)\n"
            "            depth += 1\n"
            "        # Print response body if available\n"
            "        if hasattr(exc, 'response') and exc.response is not None:\n"
            "            r = exc.response\n"
            "            print(f'[LLM_ERROR] status={getattr(r, \"status_code\", \"?\")}, body={getattr(r, \"text\", \"\")[:2000]}', flush=True)\n"
            "        # Print traceback for connection errors\n"
            "        if 'onnect' in str(exc).lower():\n"
            "            import traceback\n"
            "            traceback.print_exc()\n"
            "    _rm.RetryMixin.log_retry_attempt = _patched_log\n"
            "    print('[HiClaw] retry_mixin patched for detailed error logging', file=sys.stderr)\n"
            "except Exception as e:\n"
            "    print(f'[HiClaw] retry patch failed: {e}', file=sys.stderr)\n"
            "#\n"
